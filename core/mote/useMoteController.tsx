...
import {SubscribePropsType, TopicWithOptionsType} from "@/core/mote/packets/SubscribePacket";
    ...
    const start = async () => {
      ...
      if (startup) {
        ...
        client.addEventListener("close", () => {
          ...
          outgoingUnsubscribes.current.forEach(({timeout}) => window.clearTimeout(timeout));
          outgoingMessages.current.forEach(({timeout}) => window.clearTimeout(timeout));
          outgoingSubscribes.current.forEach(({timeout}) => window.clearTimeout(timeout));
          if (expectClose) {
            return
          }
