...
export default function useFormView(children: ReactNode[]): FormViewControllerType {
  ...
  const leftOnResize = useCallback((i: number, w: number) => setLeftWidths(widths => {
    const newWidths = [...widths];
    newWidths[i] = w + 1;
    return newWidths;
  }), [setLeftWidths]);
  const rightOnResize = useCallback((i: number, w: number) => setRightWidths(widths => {
    const newWidths = [...widths];
    newWidths[i] = w + 1;
    return newWidths;
  }), [setRightWidths]);
  ...
  return {
    ...
    childPairs, onLayout, isWrapped, leftWidth, rightWidth, onLeftWidthChange: leftOnResize, onRightWidthChange: rightOnResize,
  }
}